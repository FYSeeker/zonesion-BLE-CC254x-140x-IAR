...
void NPI_InitTransport( npiCBack_t npiCBack )
{
  ...
  uartConfig.baudRate             = HAL_UART_BR_38400;//NPI_UART_BR;               //²¨ÌØÂÊ(38400)
  uartConfig.flowControl          = NPI_UART_FC;               //¹Ø±ÕÁ÷¿Ø
  uartConfig.flowControlThreshold = NPI_UART_FC_THRESHOLD;     //Á÷¿ØãÐÖµ,µ±¿ªÆôÁ÷¿ØÊ±,¸ÃÉèÖÃÓÐÐ§
  uartConfig.rx.maxBufSize        = NPI_UART_RX_BUF_SIZE;      //·¢ËÍ»º³åÇø´óÐ¡
  uartConfig.tx.maxBufSize        = NPI_UART_TX_BUF_SIZE;       //½ÓÊÕ»º³åÇø´óÐ¡
  uartConfig.idleTimeout          = NPI_UART_IDLE_TIMEOUT;     //¿ÕÏÐ³¬Ê±
  uartConfig.intEnable            = NPI_UART_INT_ENABLE;        //´®¿ÚÖÐ¶ÏÊ¹ÄÜ(´Ë´¦Îª¹Ø±Õ)
  uartConfig.callBackFunc         = (halUARTCBack_t)npiCBack;//´®¿Ú½ÓÊÕ»Øµ÷º¯Êý

  // start UART
  // Note: Assumes no issue opening UART port.
  (void)HalUARTOpen( NPI_UART_PORT, &uartConfig );       //¸ù¾ÝÉÏÊöÅäÖÃ´ò¿ª´®¿Ú0
