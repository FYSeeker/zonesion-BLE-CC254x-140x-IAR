...
{ 
  LO_UINT16(SIMPLEPROFILE_CHAR_ZXBEE_UUID), HI_UINT16(SIMPLEPROFILE_CHAR_ZXBEE_UUID)
};
...
static uint8 simpleProfileChar1UserDesp[9] = "ZXSerial\0";
...
uint8 *pZXBeeNotifyData = NULL;
void ZXBeeNotify(uint8 *buf, uint8 len);
                            ...
                            uint8 *pValue, uint8 *pLen, uint16 offset, uint8 maxLen )
{
  ...
  if ( pAttr->type.len == ATT_BT_UUID_SIZE )
  {
    ...
    switch ( uuid )
    {
    case SIMPLEPROFILE_CHAR_ZXBEE_UUID:
      {
       uint16 fromSerial(uint8 *pdat, uint8 len);
        *pLen = fromSerial(pValue, maxLen);
  ...
  if ( pAttr->type.len == ATT_BT_UUID_SIZE )
  {
    ...
    switch ( uuid )
    {
      case SIMPLEPROFILE_CHAR_ZXBEE_UUID:
        ...
        if ( offset == 0 )
        {
          void toSerial(uint8 *pdat, uint8 len);
          toSerial(pValue, len);
        }
        else
        {
...
void ZXBeeNotify(uint8 *buf, uint8 len)
{
  ...
  uint16 ch = simpleProfileAttrTbl[2].handle;
  if (ch != 0) {
    attHandleValueNoti_t an;
    an.handle = ch;
    an.len = len;
    osal_memcpy(an.value, buf, len);
    GATT_Notification(0, &an, FALSE);
  }
}
/*********************************************************************
*********************************************************************/
