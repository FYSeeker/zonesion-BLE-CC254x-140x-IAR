...
void MyEventProcess(uint16 events );
void at_proc(void);
void at_init(void);
/*********************************************************************
 * MACROS
...
#define DEFAULT_DESIRED_MIN_CONN_INTERVAL     80   //80

// Maximum connection interval (units of 1.25ms, 800=1000ms) if automatic parameter update request is enabled
#define DEFAULT_DESIRED_MAX_CONN_INTERVAL     150  //800
...
{ 
  ...
  /*
  0x02, 0x01, 0x1a, 0x1a, 0xff, 0x4c, 0x00, 0x02, 0x15,  // Apple's fixed iBeacon advertising prefix
  ...
  0x90,0x69,0xBD,0xB8,0x8C,0x11,0x41,0x6B,0xAC,0x3F,0x33,0x46,0x8C,0x27,0x88,0xA3,
  0x04,0x4B,
  0xc5*/
};
...
static void simpleProfileChangeCB( uint8 paramID, uint8* pdat, uint8 len);
...
#ifdef XLAB  
  if ( events & 0x00f0 ){
    uint16 e = events & 0x00f0;    
    if (e & 0x0070) {
#ifndef CC2540_Serial
      MyEventProcess(e&0x0070);
#endif
    }
    if (e & 0x0080){
      at_proc();
...
static void performPeriodicTask( void )
{
   void bleInfPoll(void);
     ...
     * function is called.
     */
    //SimpleProfile_SetParameter( SIMPLEPROFILE_CHAR_ZXBEE, sizeof(uint8), &valueToCopy);
}
